...
1. Record the version and current directory:
...
2. Validate and run doctor with the exact config path:
...
Re-run `axocoatl service install --config ...` when the binary or config path
