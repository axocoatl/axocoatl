...
- Keep credentials in the process environment or an approved OS secret
  injection mechanism; Axocoatl does not load `.env` automatically.
- The onboarding wizard may place a supplied key in `.env.example`; move it,
  restrict it, and do not commit it.
